...
export function QuestionCard({ question, onEdit, onDelete, onClick, sortable = false }: QuestionCardProps) {
  const key = (question.questionType ?? '').toLowerCase()
  ...
  const renderInner = (drag?: {
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        <CardBody question={question} meta={meta} />
      </div>
    </div>
  )
...
function CardBody({ question, meta }: { question: Question; meta: TypeMeta }) {
  const key = (question.questionType ?? '').toLowerCase()
  switch (key) {
    case 'multiple_choice':
      return <MultipleChoiceBody question={question} meta={meta} />
    case 'fill_blank':
      return <FillBlankBody question={question} meta={meta} />
    case 'matching':
      return <MatchingBody question={question} meta={meta} />
    case 'ordering':
      return <OrderingBody question={question} meta={meta} />
    case 'translation':
      return <TranslationBody question={question} meta={meta} />
    case 'listening':
    case 'speaking':
      return <AudioBody question={question} meta={meta} />
    default:
      return <Row label="Nội dung">{question.question || '—'}</Row>
  }
}
...
function MultipleChoiceBody({ question, meta }: { question: Question; meta: TypeMeta }) {
  const opts = question.options as { choices?: string[] } | string[] | null | undefined
  const choices: string[] = Array.isArray(opts)
    ? (opts as string[])
    : Array.isArray(opts?.choices)
      ? opts.choices
      : []
  const correct = question.correctAnswer as { selectedChoice?: string } | string | null
...
function FillBlankBody({ question, meta }: { question: Question; meta: TypeMeta }) {
  const opts = question.options as { sentence?: string; acceptedAnswers?: string[][] } | null | undefined
...
function MatchingBody({ question, meta }: { question: Question; meta: TypeMeta }) {
  const opts = question.options as { pairs?: Array<{ left?: string; right?: string }> } | null | undefined
...
function OrderingBody({ question, meta }: { question: Question; meta: TypeMeta }) {
  const opts = question.options as { items?: string[] } | null | undefined
...
function TranslationBody({ question, meta }: { question: Question; meta: TypeMeta }) {
  const opts = question.options as {
    sourceText?: string
    sourceLanguage?: string
    targetLanguage?: string
  } | null | undefined
  const correct = question.correctAnswer as { translation?: string } | null
...
function AudioBody({ question, meta }: { question: Question; meta: TypeMeta }) {
  const opts = question.options as {
    transcriptType?: string
    keywords?: string[]
    promptText?: string
  } | null | undefined
  const correct = question.correctAnswer as { transcript?: string } | null
