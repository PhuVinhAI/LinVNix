...
import type { ExerciseFormHandle } from './exercise-forms/types'
...
function deriveCommon(initial: Record<string, unknown> | undefined | null): CommonState {
  return {
    exerciseType: String(initial?.exerciseType ?? 'multiple_choice').toLowerCase(),
...
export function ExerciseFormPage({ mode }: { mode: 'create' | 'edit' }) {
  ...
  const [common, setCommon] = useState<CommonState>(() =>
    deriveCommon(exercise as unknown as Record<string, unknown> | null),
  )
  const handleRef = useRef<ExerciseFormHandle | null>(null)

  useEffect(() => {
    if (mode === 'edit' && exercise) {
      setCommon(deriveCommon(exercise as unknown as Record<string, unknown>))
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [exercise?.id])

  const typeMeta = useMemo(() => TYPES.find((t) => t.value === common.exerciseType) ?? TYPES[0], [common.exerciseType])

  const updateCommon = <K extends keyof CommonState>(key: K, value: CommonState[K]) => {
    setCommon((prev) => ({ ...prev, [key]: value }))
  }

  const handleFormChange = useCallback((handle: ExerciseFormHandle) => {
    handleRef.current = handle
  }, [])

  const backPath = setId ? learningPath.exerciseSet(setId) : learningPath.courses()

  // The form provides its own initial; re-rendering it with a new key when
  // the type changes ensures a clean draft state.
  const initialForForm = exercise as unknown as Record<string, unknown> | null
  const formKey = `${exercise?.id ?? 'new'}-${common.exerciseType}`
  ...
  const save = async () => {
    ...
    try {
      ...
      // Speaking expects the audio URL under promptAudioUrl; listening under audioUrl.
      const opts = (payload.options ?? {}) as Record<string, unknown>
      if (common.exerciseType === 'listening') {
        opts.audioUrl = common.questionAudioUrl || ''
      } else if (common.exerciseType === 'speaking') {
        opts.promptAudioUrl = common.questionAudioUrl || ''
      }

      const body: Record<string, unknown> = {
        exerciseType: TYPE_TO_ENUM[common.exerciseType] ?? common.exerciseType.toUpperCase(),
      ...
      if (mode === 'edit' && id) {
        await mutations.updateExercise.mutateAsync({ id, payload: body })
    ...
    <div className="flex min-h-full flex-col">
      <div className="space-y-6 flex-1">
        ...
        <div className="flex flex-wrap items-center gap-2 rounded-full border-2 border-border bg-card px-2 py-2">
          <TypePicker value={common.exerciseType} onChange={(v) => updateCommon('exerciseType', v)} />
          <Divider />
