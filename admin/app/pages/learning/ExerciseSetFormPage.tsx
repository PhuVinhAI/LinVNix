...
export function ExerciseSetFormPage({ mode }: { mode: 'create' | 'edit' }) {
  const { lessonId, id } = useParams()
  const navigate = useNavigate()
  const { data: lesson } = useAdminLesson(lessonId)
  const mutations = useLearningAdminMutation()

  const initialValue = lesson?.exerciseSets?.find((item) => item.id === id)
    ...
    <div className="space-y-6">
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-end gap-2 pt-4 border-t-2 border-border">
          ...
          <Button type="submit" form="exercise-set-form">
