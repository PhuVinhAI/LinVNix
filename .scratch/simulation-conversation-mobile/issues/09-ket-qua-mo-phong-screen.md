...
Build the **Kết quả mô phỏng** screen. Route: `/practice/results/:id` (outside shell, push). Accessible from: chat completed state "Xem kết quả" button, results history tap.
...
- Actions: "Chơi lại" (`AppButton.primary` → push character selection with previous character pre-selected) + "Xem lại hội thoại" (`AppButton.outline` → push chat history with `isHistory: true`)
- For non-COMPLETED end reasons: context-appropriate messaging. TOO_MANY_ERRORS → study suggestion ("Hãy ôn lại bài học trước khi thử lại"), no replay. INAPPROPRIATE/ABUSIVE → warning, no replay button.
...
- [ ] "Chơi lại" pushes to character selection with previous character pre-selected
- [ ] "Xem lại hội thoại" pushes to chat screen in `isHistory: true` mode
- [ ] TOO_MANY_ERRORS end reason: shows study suggestion, no "Chơi lại" button
- [ ] INAPPROPRIATE/ABUSIVE end reason: shows warning, no "Chơi lại" button
