...
1. **Thanh Trợ lý AI** (thu gọn, luôn hiện ở đáy khi bật) — chưa mở. Tap → mở Trạng thái Hỏi; long-press → mở trực tiếp Trạng thái Toàn màn hình. Có thể ẩn qua cài đặt profile.
2. **Trạng thái Hỏi** (mid) — single-exchange focused, không hiện message list, chỉ text (không ảnh). Có 4 phase tuần tự: **Soạn** (textarea ≤ 5 dòng + nút Gửi), **Loading** (spinner + "Đang suy nghĩ..." + nút Dừng), **Đọc** (AI streaming render markdown, tự co giãn tới max ~75% screen + nút Dừng khi đang stream / nút Soạn tiếp khi đã xong), **Lỗi** (thông báo + nút Thử lại). Tap **Soạn tiếp** → quay lại phase Soạn, AI message bị ẩn (Hội thoại server-side vẫn liên tục).
3. **Trạng thái Toàn màn hình** (full) — chat list truyền thống, hoạt động độc lập với Trạng thái Hỏi. Có 4 phase riêng: **Soạn** (compose bar luôn sẵn + nút camera/ảnh + preview ảnh + nút Gửi), **Loading** (typing indicator inline trong message list + tool displayName), **Đọc** (AI bubble streaming render markdown inline + proposal cards inline), **Lỗi** (error bubble inline + nút Thử lại). Compose bar luôn hiện; không có nút "Soạn tiếp". Gửi được ảnh (tối đa 4 ảnh/tin, mỗi ảnh ≤10MB). Tin nhắn hỗ trợ hành động: copy (user + AI), regenerate (user — xoá tin từ điểm đó + gửi lại), edit (user — sửa inline + branch mới). Đóng Full → Collapsed, kết thúc phiên. Sidebar: drawer trái với list **Hội thoại** + CRUD reactive (rename, delete, + Mới cập nhật ngay) + thanh tìm kiếm theo title + lazy load tin nhắn (20 tin/trang).

Học viên muốn xem lại Tin nhắn trước cùng Hội thoại → vào Trạng thái Toàn màn hình. Trạng thái Hỏi cố tình giữ tối giản — chỉ text Q&A nhanh.
_Avoid_: AI Assistant, Chatbot, Cô giáo AI, Gia sư AI

**Thanh Trợ lý AI**:
Thanh ngang nhỏ ở vị trí **thấp nhất của màn hình** (dưới cả bottom nav trên route có bottom nav). Hiện trên mọi route authenticated kể cả khi đang làm bài tập, trừ khi **Học viên** tắt qua cài đặt profile (`assistant_bar_visible`). Không hiện ở splash, các route auth (`/login`, `/register`, `/verify-email`, `/forgot-password`, `/reset-password*`), và `/onboarding`. Tap → mở **Trợ lý AI** Trạng thái Hỏi; long-press → mở trực tiếp Trạng thái Toàn màn hình. Có nút **Reset** để ngừng active **Hội thoại** hiện tại và mở phiên mới. Khi ẩn, không có entry point thay thế — tính năng bị tắt hoàn toàn.
...
**Tin nhắn**:
Một lượt trong **Hội thoại** — do học viên gửi, AI phản hồi, hoặc kết quả gọi công cụ AI. Học viên chỉ thấy tin nhắn USER và ASSISTANT; tin nhắn TOOL là nội bộ agent loop. Nội dung tin nhắn được lưu dạng structured JSONB (cột `content` kiểu jsonb): mảng các phần tử `{ type: "text", text }` hoặc `{ type: "image", url }`. Tin nhắn user có thể đính kèm tối đa 4 ảnh. Trong Trạng thái Toàn màn hình, tin nhắn hỗ trợ hành động: copy (raw text/markdown), regenerate (user — xoá từ điểm đó + gửi lại), edit (user — sửa inline + branch mới). Trong Trạng thái Hỏi chỉ hỗ trợ select text hệ thống.
...
- **Trạng thái Toàn màn hình hoạt động độc lập, không phụ thuộc Trạng thái Hỏi** — Full có state machine riêng (9 states: Collapsed, MidCompose/Loading/Reading/Error, FullCompose/Loading/Reading/Error). `AssistantFull.priorState` bị loại bỏ. Đóng Full = Collapsed (không quay Mid). Reset/"+ Mới" trong Full → FullCompose (ở lại Full). Long-press Thanh Trợ lý AI → FullCompose trực tiếp. Mirror khi Mid→Full: MidCompose→FullCompose, MidLoading→FullLoading, MidReading(streaming)→FullReading(streaming), MidError→FullError. Stream tiếp tục không gián đoạn.
- **Tin nhắn content dạng structured JSONB** — cột `ConversationMessage.content` chuyển từ `text` sang `jsonb`. Format: `[{ type: "text", text: "..." }, { type: "image", url: "/uploads/images/abc.jpg" }]`. Migration: text cũ → `[{ type: "text", text: "<old>" }]`. Code đọc phải handle cả 2 format trong transition.
- **Ảnh chỉ gửi được trong Trạng thái Toàn màn hình** — Trạng thái Hỏi chỉ text. Full compose bar có nút camera/ảnh + preview row (thumbnail + nút X). Tối đa 4 ảnh/tin, mỗi ảnh ≤10MB. Ảnh trong message bubble: row ngang cuộn được, tap zoom fullscreen. Upload: 1 call multipart/form-data tới `POST /ai/chat/stream`. Backend lưu qua `StorageService.uploadImage()`, gửi inline base64 cho Gemini Interactions API.
- **Message actions trong Full: copy, regenerate, edit** — ẩn mặc định, hiện khi tap message, tap ngoài → ẩn. Copy = raw text/markdown. Regenerate (user message) = xoá tin từ điểm đó + gửi lại cùng text. Edit (user message) = sửa inline (bubble → text field) + branch mới. Cả regenerate/edit cancel stream đang chạy. Backend: endpoint mới `DELETE /ai/conversations/:id/messages?from=<messageId>` (hard-delete từ điểm đó, ownership-checked).
- **Sidebar CRUD reactive + search + lazy load** — rename/delete/create cập nhật list ngay qua Riverpod invalidate. Search: thanh tìm kiếm filter theo title local. Lazy load tin nhắn: 20 tin gần nhất, scroll lên tải thêm 20. Backend: pagination `GET /ai/conversations/:id/messages?before=<cursor>&limit=20`.
- **Thanh Trợ lý AI có thể ẩn** — preference `assistant_bar_visible` (bool, default true) trong PreferencesService, toggle ở màn profile. Ẩn = tắt tính năng, không entry point thay thế. Toggle khi stream active → đợi xong mới ẩn.
