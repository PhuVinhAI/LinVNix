...
void main() {
  ...
  group('happy-path trigger sequence (PRD §"Mobile UI state machine")', () {
    ...
    test('full-screen wraps the active mid state while a turn streams', () {
      sm.openBar();
      sm.enterFull();

      sm.send('from full');
      var full = container.read(assistantStateMachineProvider) as AssistantFull;
      expect(full.priorState, isA<AssistantMidLoading>());

      sm.onTextChunk('answer');
      full = container.read(assistantStateMachineProvider) as AssistantFull;
      final reading = full.priorState as AssistantMidReading;
      expect(reading.partial, 'answer');
      expect(reading.streaming, isTrue);

      sm.onDone(messageId: 'm-full', interrupted: false);
      full = container.read(assistantStateMachineProvider) as AssistantFull;
      final done = full.priorState as AssistantMidReading;
