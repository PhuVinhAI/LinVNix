...
for (const course of data.courses) {
  ...
  course.modules.forEach((mod: any, mIdx: number) => {
    const lessonTypes = mod.lessons.map((l: any) => l.lesson_type);
    console.log(`  Module ${mIdx + 1}: "${mod.title}"`);
    console.log(`    Lessons (${mod.lessons.length}): ${lessonTypes.join(', ')}`);
    
    mod.lessons.forEach((les: any, lIdx: number) => {
      console.log(`      Lesson ${lIdx + 1}: "${les.title}" [${les.lesson_type}]`);
